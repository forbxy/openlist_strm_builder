...
- 生成kodi蓝光文件夹strm(当strm文件内容格式为kodi_webdav或kodi_webdav_noauth时可选，开启后当识别到当前目录下有bdmv目录，且bdmv目录下有index.bdmv，
则使用当前目录名作为strm的名字，strm内容为index.bdmv的路径，且不再继续遍历当前文件夹或子文件夹，默认关闭)
